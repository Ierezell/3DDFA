...
"""
The pipeline of 3DDFA prediction: given one image,
predict the 3d face vertices, 68 landmarks and visualization.

[todo]
1. CPU optimization:
https://pmchojnacki.wordpress.com/2018/10/07/slow-pytorch-cpu-performance
"""
...
from utils.inference import (get_suffix, parse_roi_box_from_landmark,
                             crop_img, predict_68pts, dump_to_ply, dump_vertex,
                             draw_landmarks, predict_dense,
                             parse_roi_box_from_bbox, get_colors,
...
def main(args):
    ...
    transform = transforms.Compose([ToTensorGjz(), NormalizeGjz(mean=127.5,
                                                                std=128)])
    for img_fp in args.files:
        ...
        for rect in rects:
            # whether use dlib landmark to crop image,
            # if not, use only face bbox to calc roi bbox for cropping
            if args.dlib_landmark:
            ...
            # forward: one step
            img = cv2.resize(img, dsize=(STD_SIZE, STD_SIZE),
            ...
            # PAram de taille 1*62 car 62 classes demandé en init du modele
            # 68 pts
            ...
            if args.bbox_init == 'two':
                ...
                img_step2 = cv2.resize(img_step2, dsize=(STD_SIZE, STD_SIZE),
                                       interpolation=cv2.INTER_LINEAR)
            ...
            if (args.dump_ply or args.dump_vertex or args.dump_depth or
                    args.dump_pncc or args.dump_obj):
                vertices = predict_dense(param, roi_box)
                vertices_lst.append(vertices)
            if args.dump_ply:
                dump_to_ply(vertices, tri,
                            '{}_{}.ply'.format(img_fp.replace(suffix, ''),
                                               ind))
            if args.dump_vertex:
                dump_vertex(vertices,
                            '{}_{}.mat'.format(img_fp.replace(suffix, ''),
                                               ind))
            if args.dump_pts:
        ...
        if args.dump_pose:
            # P, pose = parse_pose(param)  # Camera matrix (without scale),
            # and pose (yaw, pitch, roll, to verify)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=img_fp.replace(
                suffix, '_3DDFA.jpg'), show_flg=args.show_flg)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='3DDFA inference pipeline')
    parser.add_argument('-f', '--files', nargs='+',
                        help="""image files paths fed into network,
    ...
    parser.add_argument('--bbox_init', default='one', type=str,
                        help="""one|two: one-step bbox
                        initialization or two-step""")
