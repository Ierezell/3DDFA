...
_load_cpu = _load
def _numpy_to_tensor(x): return torch.from_numpy(x)
def _tensor_to_numpy(x): return x.cpu()
def _numpy_to_cuda(x): return _tensor_to_cuda(torch.from_numpy(x))
def _cuda_to_tensor(x): return x.cpu()
def _cuda_to_numpy(x): return x.cpu().numpy()
